...
import { addBulkCategories } from "../../../services/ApiService";
...
const CategoryBulkUpload: React.FC<CategoryBulkUploadProps> = ({ isOpen, onClose, onUpload }) => {
    ...
    const REQUIRED_HEADERS_KEYS = [
        "categoryName", "categoryShortCode", "categoryDescription"
    ];
    ...
    const handleDownloadSample = () => {
        const sampleData = [
            {
                ...
                categoryShortCode: "EL",
                categoryDescription: "All electronics components"
            },
            {
                categoryName: "Furniture",
                categoryShortCode: "FU",
                categoryDescription: "Office furniture"
            }
        ];
        ...
        XLSX.writeFile(workbook, "Category_Bulk_Upload_Sample.xlsx");
    };
    ...
    const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        reader.onload = (event) => {
            try {
                ...
                // Check for missing headers. Ensure at least categoryName is there.
                // We will relax strict validation so users can upload even if they miss description, and let backend handle it
                const missingHeaders = REQUIRED_HEADERS_LOWER.filter((required) => !fileHeadersLower.includes(required));

                // We only strictly require CategoryName and CategoryShortCode for basic mapping check, 
                // but actually the sample specifies all 3. Let's enforce it to prevent bad requests.
                ...
                const normalizedData = json.map(row => {
                    ...
                    // Format constraints: Short code max 2 chars, uppercase
                    if (newRow.categoryShortCode) {
                        newRow.categoryShortCode = newRow.categoryShortCode.toUpperCase().replace(/[^A-Z]/g, "").slice(0, 2);
                    }
                    return newRow;
                });
    ...
    const handleSubmit = async () => {
        setIsProcessing(true);
        try {
            const res = await addBulkCategories(excelData);
            const body = res.data;
            ...
            toast.success(body?.message || "Categories processed");
        } catch (error: any) {
    ...
    // Calculate created and failed from our specific response format
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-xl shadow-2xl max-w-5xl w-full mx-4 max-h-[90vh] overflow-hidden flex flex-col">
                ...
                <div className="px-6 py-4 border-b bg-blue-50 flex justify-between items-center">
                    <div>
                        <h2 className="text-xl font-bold text-blue-700">Bulk Upload Categories</h2>
                                            ...
                                            <table className="min-w-full divide-y divide-gray-200 border border-gray-200">
                                                <thead className="bg-gray-50 sticky top-0">
                                                    <tr>
                                                        ...
                                                        <th className="px-4 py-3 text-left text-xs font-medium text-gray-700 uppercase">Short Code</th>
                                                ...
                                                <tbody className="bg-white divide-y divide-gray-200">
                                                    {successRecords.map((item: any, idx: number) => (
                                                        <tr key={idx} className="hover:bg-gray-50">
                                                            ...
                                                            <td className="px-4 py-3 text-sm text-gray-600 font-mono">{item.categoryShortCode}</td>
                                                            <td className="px-4 py-3 text-sm text-gray-500 font-mono">{item.data?.categoryId || item.data?.id || '-'}</td>
                                                        </tr>
                                                    ))}
                                                </tbody>
                                            </table>
                                                ...
                                                <tbody className="bg-white divide-y divide-gray-200">
                                                    {failedRecords.map((item: any, idx: number) => (
                                                        <tr key={idx} className="hover:bg-red-50">
                                                            ...
                                                            <td className="px-4 py-3 text-sm text-gray-600 font-mono">{item.categoryShortCode || "N/A"}</td>
                                                        </tr>
                                                    ))}
                                                </tbody>
                                            </table>
